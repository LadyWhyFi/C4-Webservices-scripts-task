...
def get_weather_station_location():
	return "Weather station: Oizy - Belgium"
server.register_function(get_weather_station_location)

def get_weather_station_coordinates():
	return "Coordinates: 49.8966N-5.011E"
server.register_function(get_weather_station_coordinates)

def get_date():
	return 'conditions recorded on:', datetime.datetime.now()
server.register_function(get_date)

def get_line():
	return "=" * 50
server.register_function(get_line)

def get_temperature():
	return random.randint(-20,40), 'degrees Celcius'
...
def get_wind_speed():
	return random.randint(0,12), 'Beaufort'
